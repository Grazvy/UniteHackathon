...
with torch.no_grad():
    for _, cust in customers_test.iterrows():
        ...
        probs      = torch.sigmoid(e_logits)[0].cpu().numpy()    # (N_ECLASS,)
        date_probs = torch.softmax(d_logits, dim=-1)[0].cpu().numpy()  # (6,)
        ...
        for idx in top_idx:
            if count >= TOP_K or probs[idx] < MIN_PROB:
                break
            # Economic filter: expected savings over 6 months > 6 × fee
